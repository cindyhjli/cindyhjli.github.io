...
const Project = ({term, title, description, skills, priv, link}) => {
  return (
      <div className='relative h-auto 2xl:w-[49%] shadow-xl bg-[#292929] shadow-[#111111] rounded-xl p-10 hover:scale-105 ease-in duration-300'>
        ...
        <p className='text-gray-200 text-4xl mt-2 font-bold'>
            {title}
        </p>
        {link ? (
          <a href={link} className='flex pt-2 text-[#19A6FF] hover:underline font-semibold text-2xl'>View Project<span className='pt-1 pl-2'><FaExternalLinkAlt /></span></a>
        ) : null}
        {priv ? (
          <p className='text-2xl pt-1 text-gray-400'>(Private repository)</p>
        ) : null}
